...
class snapshot {
    public:
        snapshot( const fs::path& file_name ): snapshot( file_name, ".", true ) {};
        HASH_SNAPSHOT_EXPORT snapshot( const fs::path& file_name, const fs::path& dir, bool follow_symlinks = true );
